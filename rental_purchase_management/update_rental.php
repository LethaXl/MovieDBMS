...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update'])) {
        ...
        $identifierColumn = 'RENTAL_ID';

        if ($field === 'RENTAL_ID' || $field === 'MOVIE_ID' || $field === 'TRANSACTION_ID') {
            if (!is_numeric($newValue)) {
                $error = "New value must be a number for $field.";
            }
            $newValue = (int)$newValue;
            $identifierValue = (int)$identifierValue;
        } elseif ($field === 'RENTAL_START_DATE' || $field === 'RENTAL_END_DATE') {
        ...
        $sql = "UPDATE rental SET $field = :newValue WHERE $identifierColumn = :identifierValue";
...
<head>
    ...
    <title>Update or Delete Rental</title>
    <style>
        body {
    ...
    <div class="form-container">
        <form method="POST" action="">
            ...
            <select id="field_name" name="field_name" required>
                <option value="RENTAL_ID">RENTAL_ID</option>
                <option value="MOVIE_ID">MOVIE_ID</option>
                <option value="TRANSACTION_ID">TRANSACTION_ID</option>
                <option value="RENTAL_STATUS">RENTAL_STATUS</option>
                <option value="RENTAL_START_DATE">RENTAL_START_DATE</option>
                <option value="RENTAL_END_DATE">RENTAL_END_DATE</option>
            </select>

            <label for="new_value">New value:</label>
            <input type="text" id="new_value" name="new_value" placeholder="e.g., Active" required>

            <label for="identifier">Identifier value (e.g., 4 for RENTAL_ID):</label>
        ...
        <form method="POST" action="">
            <h2>Delete Rental</h2>
            <label for="delete_identifier">Rental ID to delete:</label>
